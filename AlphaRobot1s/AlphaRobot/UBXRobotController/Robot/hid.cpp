...
#if  0
    d->disconnected = 1;
    CFRunLoopStop(d->run_loop);  // 原代码，现加上判断d->device_handle是由于拨usb时机率性挂死
#else
    if(d)
    {
        d->disconnected = 1;
        if(d->device_handle)
        {
            qDebug()<<"\nhid_device_removal_callback11\n";
//            CFRunLoopStop(d->run_loop); //asu, hid_close可能先调用，从而释放后再调用而挂死
        }
    }
#endif
