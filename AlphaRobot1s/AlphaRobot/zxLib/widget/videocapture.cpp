...
CVideoCapture::~CVideoCapture()
{
    ...
    SAFE_DELETE(m_pVideoProbe);

    if(m_bOkSetProbeToMediaPlayer)
    {
        SAFE_DELETE(m_pVideoItem);//m_mediaPlayer own it
    }
}
                                 ...
                                 const int nOuttimeMsec)
{
    ...
    QTimer::singleShot(UBXWidgetConstants::VIDEO_CAPTURE_INTERNAL, this, &CVideoCapture::slotOnTryCaptureTime);
...
//     unsigned char* uv = pNV12 + y_size;

//     //!分配YUV420P空间
//     pOutYuv = new uchar[y_size*3/2];
//     if(NULL == pOutYuv)
//         return false;
//     memset(pOutYuv,0,y_size*3/2);

//     unsigned char* y_tmp = pOutYuv;
//     unsigned char* u_tmp = pOutYuv + y_size;
//     unsigned char* v_tmp = pOutYuv + y_size * 5 / 4;
