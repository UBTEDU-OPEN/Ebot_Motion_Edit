...
void UBXWidget::setTitleBarHeight(int titleBarHeight)
{
    if(0 <= titleBarHeight)
    {
...
{
    if(m_bEnableMove && m_bLeftDown)
    {
        QRect rectTitleBar(0, 0, this->width(), m_nTitleBarHeight);
        if(rectTitleBar.contains(mapFromGlobal(m_ptPre)))
        {
            QPoint nowPt = event->globalPos();
            QPoint moveTo = this->frameGeometry().topLeft() + (nowPt - m_ptPre);
            move(moveTo);
            m_ptPre = nowPt;
        }
    }
...
void UBXWidget::changeEvent(QEvent *event)
{
