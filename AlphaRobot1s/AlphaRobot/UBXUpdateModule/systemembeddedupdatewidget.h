...
class UBXUPDATEMODULESHARED_EXPORT SystemEmbeddedUpdateWidget : public QDialog,  public zxObserverBase
{
    Q_OBJECT
...
signals:
    ...
    *****************/
    ...
    // 嵌入式版本信息请求结果
    void sigOnGetSystemEmbedVersionInfo(const QString &strCurrentVersion, const QString &strLatestVersion, int nStatus);
...
public slots:
    void slotOnTypeChange(int eUbtProductTypeOld, int eUbtProductTypeNew);
...
private:
    ...
    */
    ...
    //手动升级
    bool processRobotManalUpGrade(WPARAM wParam, LPARAM lParam);
    //获取自动升级进度
    bool processRobotGetAutoUpdadeProgress(WPARAM wParam, LPARAM lParam);
    //是否安装升级通知
    bool processRobotIsInstallingUpdate(WPARAM wParam, LPARAM lParam);
...
private:
    ...
    emCheckUpdateType m_eRobotUpdateType;  // 版本类型
    QString m_strLocalSysEmbedVersion; // 当前嵌入式版本, robot soft version
    ...
    int m_nReqServerVersionTimerID; // 请求后台版本的定时器ID
