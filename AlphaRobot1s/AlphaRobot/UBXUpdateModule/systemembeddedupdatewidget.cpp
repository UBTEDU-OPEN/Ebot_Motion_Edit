...
SystemEmbeddedUpdateWidget::SystemEmbeddedUpdateWidget(QWidget *parent) :
    QDialog(parent),  m_eRobotUpdateType(eCheckUpdateTypeOldRobot1S),
...
* 作者: hwx
*/
void SystemEmbeddedUpdateWidget::onRemoveDevice()
{
    // 如果是强制升级移除设备，则忽略
    ...
    if (m_pProgressDialog && m_pProgressDialog->isVisible())
    {
        slotOnCancelUpdate();
...
void SystemEmbeddedUpdateWidget::timerEvent(QTimerEvent *event)
{
    ...
    else if (m_nReqServerVersionTimerID == nIDEvent)
    {
        killTimer(m_nReqServerVersionTimerID);
        m_nReqServerVersionTimerID = 0;
        slotUpdateUI();
        CMessageBox::critical(NULL, tr(""), tr("Request for firmware version on server timeout."));
    }
}
...
void SystemEmbeddedUpdateWidget::slotOnTypeChange(int eUbtProductTypeOld, int eUbtProductTypeNew)
{
    int nold = eUbtProductTypeOld;
    int nE = eUbtProductTypeNew;
}
...
void SystemEmbeddedUpdateWidget::slotOnUpdate()
{
    if(eUbtProduct1E == UBXBase::getInstance()->getCurrentRobotProductType())
    {
        if (g_pRobotController)
        {
            g_pRobotController->Req_ManalUpGrade();
...
bool SystemEmbeddedUpdateWidget::downloadUpdateToLocalComputer()
{
    ...
    // 如果文件存在，则先删除文件
    if (QFile::exists(strSaveFilePath) && (!QFile::remove(strSaveFilePath)))
    {
        AlphaRobotLog::getInstance()->printError(QString("File %1 exist, and failed to remove!"));
        return false;
    }
...
//    m_pProgressDialog->setMaximum(100);
       ...
       //for temp show is updating
        CProgressDialog progressDialog;
        progressDialog.setMinimum(0);
        progressDialog.setMaximum(0);
        progressDialog.setModal(true);
        progressDialog.setCancelButton(NULL);
        progressDialog.setLabelText(tr("Checking version..."));
        progressDialog.show();
    ///////////////////////////////////////////////////////////
...
{
    if (m_strLocalSysEmbedVersion.isEmpty() || (m_eRobotUpdateType == eCheckUpateTypeUnknown))
    {
        return;
    }

    m_nReqServerVersionTimerID = startTimer(UpdateModuleConstant::TIMIEOUT_60000MS);

    if(eUbtProduct1E == UBXBase::getInstance()->getCurrentRobotProductType())
    {
        

        //m_strLocalSysEmbedVersion = "alpha1e_v0.1.5";
        
        

        CSystemUpdateHttpHandle::checkUpdate(m_strLocalSysEmbedVersion, m_LatestVersionInfo);
    }
    else
    {
    ...
    // #endif
        ...
        if (sUpdateUrl.isEmpty() || m_strLocalSysEmbedVersion.isEmpty() || (m_eRobotUpdateType == eCheckUpateTypeUnknown))
        {
            return;
        }

        CheckUpdate(sUpdateUrl, m_LatestVersionInfo, m_strLocalSysEmbedVersion, m_eRobotUpdateType);
    }

    if (m_nReqServerVersionTimerID > 0)
    {
        killTimer(m_nReqServerVersionTimerID);
    }
}
...
bool SystemEmbeddedUpdateWidget::processRobotHardwareVersion(WPARAM wParam, LPARAM lParam)
{
    ...
    bool bRobotProductKnown = true;
    bool bRobotProductTypeChanged = false;

    connect(UBXBase::getInstance(), &UBXBase::sigRobotProductTypeChange, this, &SystemEmbeddedUpdateWidget::slotOnTypeChange);
//    connect(&m_oThreadWaitUdiskShowUp, &CThreadWaitForUdiskShowup::sigIsUdiskShowUp, this, &SystemEmbeddedUpdateWidget::slotOnRobotUDiskShowUp);

 //   void sigRobotProductTypeChange(emUbtProductType eOld, emUbtProductType eNew);

    // wParam为bool类型
    if (!wParam)
    {
        m_eRobotUpdateType = eCheckUpateTypeUnknown;
        CMessageBox::critical(NULL, tr("Error"), tr("Failed to read robot version"));

        //UBXBase::getInstance()->setCurrentRobotProductType(eUbtProductUnknown);
        bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);//set to default type

       // UBXBase::getInstance()->sigRobotProductTypeChange();
    }
    else
    {
        if (strRobotType.indexOf(UpdateModuleConstant::STRING_VERSION_ALPHA1S_V, 0, Qt::CaseInsensitive) >= 0)
        {
            m_eRobotUpdateType = eCheckUpdateTypeNewRobot1S;  //更新版1S机器人端软件
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);
        }
        else if ( strRobotType.indexOf(UpdateModuleConstant::STRING_VERSION_ALPHA1P_V,  0, Qt::CaseInsensitive) >= 0)
        {
            m_eRobotUpdateType = eCheckUpdateTypeRobot1P; //更新1P机器人端软件
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);
        }
        else if (strRobotType.indexOf(UpdateModuleConstant::STRING_VERSION_ALPHA1SP_V, 0, Qt::CaseInsensitive) >= 0)
        {
            m_eRobotUpdateType = eCheckUpdateTypeRobot1SP; // 更新机器人1SP版本
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);
        }
        else if (strRobotType.indexOf(UpdateModuleConstant::STRING_VERSION_ALPHA1P_V2, 0, Qt::CaseInsensitive) >= 0)
        {
            m_eRobotUpdateType = eCheckUpdateTypeRobot1PV2; // 更新机器人1P V2.05版本
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);
        }
        else if (strRobotType.indexOf(UpdateModuleConstant::STRING_VERSION_ALPHA1P_PLUS_V2, 0, Qt::CaseInsensitive) >= 0)
        {
            m_eRobotUpdateType = eCheckUpdateTypeRobot1PPlus;  // 更新机器人1P Plus版本
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);
        }
        ...
        {
            m_eRobotUpdateType = eCheckUpdateTypeNewRobot1SV2; //更新版1SV2.0机器人端软件
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);
        }
        else if(strRobotType.indexOf(UpdateModuleConstant::STRING_VERSION_ALPHA1E_,  0, Qt::CaseInsensitive) >= 0)
        {
            //todo
            m_eRobotUpdateType = eCheckUpdateTypeRobot1E;
            bRobotProductTypeChanged = UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1E);
        }
        else
        {
            m_eRobotUpdateType = eCheckUpdateTypeOldRobot1S; //更新旧版1S机器人端软件
        }
...
//    CIniConfig::getValueRobotType(strRobotType);
//    if(strRobotType.isEmpty())
//    {
//        UBXBase::getInstance()->setCurrentRobotProductType(eUbtProduct1S1P);//set to default type
//    }
...
//#endif
    ...
    {
        //alpha1e_v1.0.0
        strLocalVersion = CSystemUpdateHttpHandle::getOnlyNumVersion(m_strLocalSysEmbedVersion);
    }
    else
    {
        //2015091004, from robot
       strLocalVersion = m_strLocalSysEmbedVersion;
    }
    if (strLocalVersion.isEmpty())
    {
        strLocalVersion =tr("Unknown");
    }

    //
    QString strLatestVersion;
    if(eUbtProduct1E == UBXBase::getInstance()->getCurrentRobotProductType())
    {
        strLatestVersion = CSystemUpdateHttpHandle::getOnlyNumVersion(m_LatestVersionInfo.m_sVersion);
    }
    else
    {
        // 获取的版本含有V，如V20170726,  from net server
        strLatestVersion = getVersionInNumber(m_LatestVersionInfo.m_sVersion);
    }

    if (strLatestVersion.isEmpty())
    {
        strLatestVersion = tr("Unknown");
    }

    ui->labLocalVersionText->setText(strLocalVersion);
    ...
    // 当前版本和服务器上版本均已知
    int nStatus = CHECKRESULT_ERROR;
    if ((!strLatestVersion.isEmpty()) && (!m_strLocalSysEmbedVersion.isEmpty()))
    {
        int nCompareVersion = -1;
        if(eUbtProduct1E == UBXBase::getInstance()->getCurrentRobotProductType())
        {
            nCompareVersion = CSystemUpdateHttpHandle::compareVersion(strLocalVersion, strLatestVersion);
        }
        else
        {
            nCompareVersion = CompareVersion(m_strLocalSysEmbedVersion, strLatestVersion, m_eRobotUpdateType);
        }

        if (nCompareVersion <= 0)
        {
            ui->pbUpdate->setEnabled(false);
            nStatus = CHECKRESULT_NOUPDATE;
        }
        else
        {
            ui->pbUpdate->setEnabled(true);
            nStatus = CHECKRESULT_NEEDUPDATE;
        }
        emit sigSystemEmbedVersionUpdateStatus(nStatus);
    }
    ...
    emit sigOnGetSystemEmbedVersionInfo(strLocalVersion, strLatestVersion, nStatus);
}
...
* 作者: hwx
*/
void SystemEmbeddedUpdateWidget::resetUI()
{
...
void SystemEmbeddedUpdateWidget::onDownloadUpdateProgress(WPARAM wParam, LPARAM lParam)
{
    ...
    if (pCurSendFileInfo->m_eType != emTranFileUpdateSysEmbed)  // 只处理文件同步
    {
        return;
    }
    ...
    {
        // 不断开会触发信号
        disconnect(m_pProgressDialog, &CProgressDialog::canceled, this, &SystemEmbeddedUpdateWidget::slotOnCancelUpdate);
        m_pProgressDialog->close();
        QString strTransFilePath = AnsiToUnicode(pCurSendFileInfo->m_szTransFilePath);
        CMessageBox::critical(NULL, tr(""), tr("Update file download to robot failed: ") + strTransFilePath);
        return;
    }
...
bool SystemEmbeddedUpdateWidget::processRobotManalUpGrade(WPARAM wParam, LPARAM lParam)
{
    if (!wParam)
    {
        // 不断开会触发信号
        ...
        if(m_bCanShowMBox)
        {
            CMessageBox::critical(NULL, tr(""), tr("Update failed"));
        }
...
bool SystemEmbeddedUpdateWidget::processRobotGetAutoUpdadeProgress(WPARAM wParam, LPARAM lParam)
{
        if (!lParam)
        {
        ...
        if (pInfo)
        {
            string strInfo = string(pInfo);
            qDebug() <<"update progress info"<< strInfo.c_str();
            ...
            if(QJsonParseError::NoError == jsonError.error)
            {
                if(parseDocument.isObject())
                {
                    ...
                    if(!obj.empty())
                    {
                        emGetAutoUpdadeProgressStatus eGetAutoUpdadeProgressStatus = (emGetAutoUpdadeProgressStatus)obj.take("status").toInt();
                        if(eUpdadeStatusFail == eGetAutoUpdadeProgressStatus)
                        {
                            if(m_pProgressDialog)
                            {
                                disconnect(m_pProgressDialog, &CProgressDialog::canceled, this, &SystemEmbeddedUpdateWidget::slotOnCancelUpdate);
                        ...
                        if(eUpdadeStatusOk == eGetAutoUpdadeProgressStatus)
                        {
                            if(m_pProgressDialog)
                            {
                                disconnect(m_pProgressDialog, &CProgressDialog::canceled, this, &SystemEmbeddedUpdateWidget::slotOnCancelUpdate);
                            ...
                            if(m_bCanShowMBox)
                            {
                                CMessageBox::critical(NULL, tr(""), tr("Update ok"));
                            }
                        }

                        if(eUpdadeStatusDowning == eGetAutoUpdadeProgressStatus)
                        {

                            int nProgress = obj.take("progress").toInt();
                            ...
                            if(!m_pProgressDialog)
                            {
                                ...
                                m_pProgressDialog->setLabelText(tr("Activating the firmware, it may takes several minutes"));
                                Sleep(10);
                            ...
                            if(m_pProgressDialog && m_bCanShowMBox)
                            {
                                int nLastProgress = m_pProgressDialog->value();
...
bool SystemEmbeddedUpdateWidget::processRobotIsInstallingUpdate(WPARAM wParam, LPARAM lParam)
{
    if(m_pProgressDialog)
    {
        disconnect(m_pProgressDialog, &CProgressDialog::canceled, this, &SystemEmbeddedUpdateWidget::slotOnCancelUpdate);
    ...
    emIsInstallingUpdateStatus eInstallStatus = (emIsInstallingUpdateStatus)wParam;

    if (eIsInstallingUpdateStatusPowerLessNoInstall == eInstallStatus)
    {
        if(m_bCanShowMBox)
        {
            CMessageBox::critical(NULL, tr(""), tr("Power is less, so install failed"));
        }
    }

    else if(eIsInstallingUpdateStatusPowerFullToInstall == eInstallStatus)
    {
        if(m_bCanShowMBox)
        {
            CMessageBox::critical(NULL, tr(""), tr("Installing"));
        }
    }
...
bool SystemEmbeddedUpdateWidget::isContainsChineseChar(const QString &strText)
{
    ...
    return CStringHandle::isContainsChineseChar(strText);
}
...
void SystemEmbeddedUpdateWidget::processTFCardCapacityRsp(WPARAM wParam, LPARAM lParam)
{
    if (!wParam && lParam)
    {
        m_pProgressDialog->close();
        CMessageBox::critical(NULL, tr(""), tr("Update failed. Robot memory has no capacity. Please remove some files to provide enough memory before update."));
        return;
    }
}
...
//    }
    ...
    else if (WM_MSG_TFCARD_NO_CAPACITY == nMsgID)
    {
        processTFCardCapacityRsp(wParam, lParam);
    }

    else if (nMsgID == WM_MSG_ROBOT_MANAL_UPGRADE_RST)
    {
        processRobotManalUpGrade(wParam, lParam);
    }
    else if (nMsgID == WM_MSG_ROBOT_GET_AUTO_UPDATE_PROGRESS_RST)
    {
        processRobotGetAutoUpdadeProgress(wParam, lParam);
    }
    else if (nMsgID == WM_MSG_ROBOT_IS_INSTATLING_UPDATE_RST)
    {
        processRobotIsInstallingUpdate(wParam, lParam);
    }
