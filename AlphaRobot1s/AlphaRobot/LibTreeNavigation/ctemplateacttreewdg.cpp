...
CTemplateActTreeWdg::CTemplateActTreeWdg(QWidget *parent):
    QTreeWidget(parent),
    m_DragItem(NULL)
{
...
CTemplateActTreeWdg::~CTemplateActTreeWdg()
{

}

void CTemplateActTreeWdg::mouseMoveEvent(QMouseEvent *event)
{
...
void CTemplateActTreeWdg::mousePressEvent(QMouseEvent *event)
{
...
//        }
        if(parent && (parent->parent() == NULL))
        {
            ...
            QString strDefActionPath = m_strTemplateDir;//CConfigs::getLocalDefaultTemplateDir();
            QDir dir(strDefActionPath);
            if(!dir.exists())
            {
                return ; //没有找到文件，即没有数据
            }
...
bool CTemplateActTreeWdg::ParseData(char *pData, int nLens)
{
    int nTotalLen = 0;
...
QString CTemplateActTreeWdg::templateDir() const
{
    return m_strTemplateDir;
}

void CTemplateActTreeWdg::setTemplateDir(const QString &strTemplateDir)
{
