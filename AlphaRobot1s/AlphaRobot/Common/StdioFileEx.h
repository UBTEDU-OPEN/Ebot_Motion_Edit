...
typedef enum TextCodeType
{
	TextCodeTypeUTF8,
	TextCodeTypeUNICODE,
	TextCodeTypeUNICODEBIGENDIAN,
	TextCodeTypeANSI,
	TextCodeTypeFILEERROR
}TextCodeType;
...
public:
	...
	//获取文件类型
	TextCodeType GetFileType();
	//按行读取文件
	BOOL     ReadString(CString& rString);
	//静态方法  获取文件类型
	static   TextCodeType GetFileType( LPCTSTR lpszFileName);
protected:
	TextCodeType m_FileType;//保存文件类型
...
protected:
	...
	//处理文件打开标志
	UINT     ProcessFlags(LPCTSTR lpszFileName, UINT& nOpenFlags,TextCodeType &tc);
};
