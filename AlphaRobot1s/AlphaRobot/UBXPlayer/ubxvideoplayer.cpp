...
bool UBXVideoPlayer::loadVideoFile(const QString &strFile)
{
    ...
    AlphaRobotLog::getInstance()->printDebug(QString("Start load video file: %1").arg(strVideoFile));

    // 先添加播放按钮，在捕获视频第一帧画面时界面就不会太单调
   addPlayItem();

   m_pMediaPlayer->setMedia(QUrl(strVideoFile));
   qDebug() << "Set media: " << strVideoFile.toStdString().c_str() << endl;

    const QString strCaptureImgPath = CConfigs::getLocalTempPath(VIDEO_CAPTURE_PREVIEW_FRAME);

#if 0
    CVideoCapture videoCapture;
    if (videoCapture.captureFrame(strVideoFile, strCaptureImgPath, VIDEO_CAPTURE_FRAME_INDEX, UBXPlayerConstants::CAPTURE_VIDEO_FRAME_TIMEOUT))
    {
        addVideoPreviewFrameItem();
    }
#else
    QString strVideoPreviewPic = strVideoFile + "?vframe/jpg/offset/0";
    bool bDownOk = CDownLoadPic::downPic(strVideoPreviewPic, strCaptureImgPath);
    if (bDownOk)
    {
        addVideoPreviewFrameItem();
    }
...
{
    m_strImageDescFile = strImageFile;
...
void UBXVideoPlayer::setBufferText(const QString &strText)
{
    if (ui->labelBuffering)
    {
        ui->labelBuffering->setText(strText);
    }
}
...
//    m_pGraphicsScene->clear();
    ...
    if (m_pMediaPlayer)
    {
        qDebug() << "Reset mediaplayer: set null" << endl;
...
void UBXVideoPlayer::resizeEvent(QResizeEvent *event)
{
    ...
    m_pGraphicsView->setFixedHeight(m_pGraphicsView->width() * 0.6);
...
void UBXVideoPlayer::showEvent(QShowEvent *event)
{
...
{
    qDebug() << "Buffer status changed: " << nPercentFilled << endl;
    if (!m_pMediaPlayer)
    {
        return;
    }
    ...
    {
        setBufferText(tr("Buffering") + QString(" %1%").arg(nPercentFilled));
    }
}
...
void UBXVideoPlayer::setUpUI()
{
    ...
    m_pGraphicsView->setSizeAdjustPolicy(QAbstractScrollArea::AdjustIgnored);
    ...
    // https://forum.qt.io/topic/43640/qmediaplayer-bufferstatus
    // http://marc.info/?l=webkit-changes&m=130016876108724&w=2
...
void UBXVideoPlayer::updateSceneRect()
{
    ...
    int nVideoOutputItemHeight = m_pGraphicsView->height();
...
/************************************
* 名称: updateImageDescItem
* 功能: 更新图片描述预览
* 返回:   void
* 时间:   2016/10/11
* 作者:   hwx
************************************/
void UBXVideoPlayer::updateImageDescItem()
{
    if (!m_pGraphpixmapDescImageItem || m_strImageDescFile.isEmpty())
    {
        return;
    }
    ...
    QPixmap pixmapImageItem;
    pixmapImageItem.fill(Qt::transparent);
    if (!QFile::exists(m_strImageDescFile))
    {
        return;
    }

    if (pixmapImageItem.load(m_strImageDescFile))
    {
        int nWidth = (int)(m_pGraphicsScene->sceneRect().width());
        int nHeight = (int) (m_pGraphicsScene->sceneRect().height());
        QPixmap pixmapScaled = pixmapImageItem.scaled(nWidth, nHeight, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
