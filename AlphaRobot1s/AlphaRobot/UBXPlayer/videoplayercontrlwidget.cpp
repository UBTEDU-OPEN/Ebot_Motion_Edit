...
//    qDebug() << "Buffer percent: " << m_pParentWidget->getPlayer()->bufferStatus() << endl;
    ...
    quint64 nPosPlayEnd = nDuration - MEIDA_PLAY_END_OFF_SET;
    QMediaService *pService = pPlayer->service();
    if (!pService)
    {
        return;
    }
    QMediaPlayerControl *pPlayerContrl = qobject_cast<QMediaPlayerControl *> (pService->requestControl("org.qt-project.qt.mediaplayercontrol/5.0"));
    if (!pPlayerContrl)
    {
        return;
    }

    QMediaTimeRange timeRanged = pPlayerContrl->availablePlaybackRanges();
    qDebug() << "Earliest time: " << timeRanged.earliestTime() << " Latest time: " << timeRanged.latestTime() << endl;
    if ((timeRanged.latestTime() < nPosPlayEnd)  && ((timeRanged.latestTime() - nPos) < PLAYER_CACHE_TIME))
    {
        m_pParentWidget->setBufferText(tr("Buffering"));
    }
    else
    {
        m_pParentWidget->setBufferText(tr(""));
    }
