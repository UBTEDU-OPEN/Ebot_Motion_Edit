...
void UBXVideoOutputItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (currentFrame.map(QAbstractVideoBuffer::ReadOnly)) {
        ...
        // 由于部分视频文件使用QGraphicsView播放时，会出现边界花屏现象，因此裁剪掉部分边界值
        QImage imageCut = imageVideo.copy(QRect(UBXPlayerConstants::VIDEO_IMAGE_ABORD_SIZE, UBXPlayerConstants::VIDEO_IMAGE_ABORD_SIZE,
                                                currentFrame.width() - UBXPlayerConstants::VIDEO_IMAGE_ABORD_SIZE * 2, currentFrame.height() - UBXPlayerConstants::VIDEO_IMAGE_ABORD_SIZE * 2));
        QImage imageScaled = imageCut.scaled(rectBounding.width(),  rectBounding.height(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
