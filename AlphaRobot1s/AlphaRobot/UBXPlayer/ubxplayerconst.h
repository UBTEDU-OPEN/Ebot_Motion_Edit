...
namespace UBXPlayerConstants
{
    ...
    const int MEDIA_PLAYER_NOTIFY_INTERNAL = 1000; // 播放器更新时间间隔
    ...
    const int PLAYER_CACHE_TIME = 2000; // 缓存播放时间，单位ms

    // 视频播放器界面上播放背景按钮的大小
    const int PLAYER_BACKGROUND_PLAY_BUTTON_WIDTH = 87;
    const int PLAYER_BACKGROUND_PLAY_BUTTON_HEIGHT = 87;

    const int CAPTURE_VIDEO_FRAME_TIMEOUT = 5000; // 捕获视频第一帧超时时间
    ...
    const int  VIDEO_IMAGE_ABORD_SIZE = 10;  // 视频图像剪切大小
