...
void DialogBlockDescription::SetStartTime(float fStartTime)
{
    m_fStartTime = fStartTime;
    QString strStartTime = QString("%1").arg(fStartTime);
    ui->lineEditStartime->setText(strStartTime);
}

void DialogBlockDescription::SetEndTime(float fEndTime)
{
    m_fEndTime = fEndTime;
    QString strEndTime = QString("%1").arg(fEndTime);
