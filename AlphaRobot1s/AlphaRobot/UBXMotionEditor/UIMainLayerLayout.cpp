...
//    }
    ...
    CUILayerManager *ui_layer_manager = (CUILayerManager *) m_pUILayerManager;
    if (ui_layer_manager)
    {
        QString strDefaultFile = tr("untitled");
        int i_tab_id = ui->tabWidgetManager->addTab(ui_layer_manager, strDefaultFile);
        ui->tabWidgetManager->setTabToolTip(i_tab_id, strDefaultFile);
        LayerManager_setTabWidgetTabText(strDefaultFile);
    }
...
void CUIMainLayerLayout::setTabWidgetTabName(QWidget *pWidget, const QString &sName)
{
    int i_tab_id = ui->tabWidgetManager->indexOf(pWidget);
    if (i_tab_id < 0)
    {
        return;
    }

    ui->tabWidgetManager->setTabText(i_tab_id, sName);
}

void CUIMainLayerLayout::setTabWidgetTabToolTip(QWidget *pWidget, const QString &sToolTip)
{
    int i_tab_id = ui->tabWidgetManager->indexOf(pWidget);
    if (i_tab_id < 0)
    {
    ...
    ui->tabWidgetManager->setTabToolTip(i_tab_id, sToolTip);
}
...
void CUIMainLayerLayout::LayerManager_setTabWidgetTabText(const QString &sFile)
{
    if (m_pUILayerManager)
    {
        m_pUILayerManager->IUILayerManager_SetTabWidgetTabText(this, sFile);
    }
}
