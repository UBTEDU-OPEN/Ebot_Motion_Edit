...
void CUILayerOptions::SetOptionHandler(ILayerOptionHandler* pHandler)
{
    ...
    {
        return;
        //        m_pOptionHandler = new COptionsMotionWidget();
    }


    if (m_pOptionHandler)
    {
        //        if (m_pOptionHandler->GetWidget()->GetSafeHwnd())
        //            m_pOptionHandler->GetWidget()->ShowWindow(SW_HIDE);
    }
    m_pOptionHandler = pHandler;
    if (m_pOptionHandler == NULL)
        return;

    //    m_pOptionHandler->GetWidget()->SetParent(this);
    //UpdateWindow();
